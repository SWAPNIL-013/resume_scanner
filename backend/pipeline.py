...
# # Configure logging once at the very start
# logging.basicConfig(
#     level=logging.INFO,
...
#         resume_json = generate_resume_json(resume_text_with_links, api_key=api_key, model=model)
#         logger.info(f"[{thread_name}-{thread_id}] ✅ Completed resume JSON generation for {filename}")

#         logger.info(f"[{thread_name}-{thread_id}] Calculating total experience from resume")
#         float_experience_years = total_experience_from_resume(resume_json.get("experience", []))
#         total_experience_years = format_experience_years(float_experience_years)
#         resume_json["total_experience_years"] = total_experience_years
#         logger.info(f"[{thread_name}-{thread_id}] Total experience years: {total_experience_years}")
...
#                 model=model,
#             )
#             logger.info(f"[{thread_name}-{thread_id}] ✅ Completed scoring for {filename}")
...
#             }
#         else:
#             logger.info(f"[{thread_name}-{thread_id}] Skipping scoring for {filename} (no JD provided)")
...
#         return {"error": {"message": str(e), "file": filename}}
...
#         "samples/Sakshi_Kusmude_Resume (4).pdf",
#         "samples/Naukri_Mr.IMRANSHARIFFHS[3y_8m].pdf",
#         "samples/Naukri_SISIRAPR[3y_0m].pdf",
#         "samples/Sachin Pandey Resume Updated.pdf",
#         "samples/NirmalJainCM_Resume.pdf",
#         "samples/Prashant_Kishore_SDE_4.5Years.pdf"
#     ]
...
#             try:
#                 res = future.result()
#                 # Add filename to the result dictionary for traceability in final output
#                 res['filename'] = file
#                 results.append(res)
#                 print(f"✅ Done processing: {file}")
#             except Exception as exc:
#                 print(f"❌ Failed processing: {file} with error: {exc}")
#                 results.append({"error": str(exc), "file": file})
