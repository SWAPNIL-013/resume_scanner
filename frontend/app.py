...
if "last_export_option" not in st.session_state:
    st.session_state.last_export_option = None
...
with st.sidebar:
    ...
    st.caption(f"⚙️ Max Workers Set To: {st.session_state.max_workers}")
...
if st.session_state.step >= 2:
    ...
    st.markdown("## **Step 2 — Job Description & Weights**")
...
# ---------------- LAYOUT: JD Section ----------------
    ...
    if current_jd.strip():
        ...
        st.markdown("### **Adjust Attribute Weights**")
        ...
        for idx, field in enumerate(fields):
            ...
            with col:
                default_val = st.session_state.weights.get(field, 0.2) * 100
        ...
        st.markdown(
            f"<p style='font-size:16px; font-weight:600;'>Total Weight: {total}%</p>",
        ...
        st.session_state.weights = {}
        st.info("No JD entered — the system will only parse resumes.")

   
    # ------------------- Evaluate Button -------------------
    st.markdown("### **Finalize & Evaluate**")
    if st.button("🚀 Evaluate Resume(s)", key="btn_evaluate", use_container_width=True):
        if not st.session_state.uploaded_paths:
            st.warning("Please upload at least one resume before evaluating.")
        else:
            headers = {}
            if st.session_state.auth_token:
                headers["Authorization"] = f"Bearer {st.session_state.auth_token}"
...
if st.session_state.step >= 3:
    ...
    if response.status_code == 200:
        ...
        failed_items  = [r for r in st.session_state.results if r.get("status") == "failed"]
        success_items = [r for r in st.session_state.results if r.get("status") == "success"]

        total = len(st.session_state.results)
        failed_names = ", ".join([item["file"] for item in failed_items]) if failed_items else "None"

        # Compact summary
        st.info(f"📄 Total resumes processed: **{total}**")
        st.success(f"✔ Successfully processed: **{len(success_items)}**")
        st.warning(f"❌ Failed: **{len(failed_items)}** — Files: {failed_names}")
...
if st.session_state.step >= 4 and st.session_state.results:
    ...
    # Only show SUCCESSFUL resumes
    success_items = [r for r in st.session_state.results if r.get("status") == "success"]

    if not success_items:
        st.warning("No successfully processed resumes to display.")
        st.stop()

    for resume in success_items:
        ...
        with st.expander(f"▶ {name}"):
            ...
            # Overall Score
            score = resume.get("score", "Score is not calculated - No JD present")
            st.markdown(
...
if st.session_state.get("step", 0) >= 4 and st.session_state.get("results"):
    st.markdown("## Step 5 — Export Options")
    
    success_items = [r for r in st.session_state.results if r.get("status") == "success"]

    if not success_items:
        st.warning("No successful resumes available for export.")
        st.stop()
    ...
    # --- Clear excel_file when switching export modes to avoid stale download button ---
    excel_export_modes = [
        "Create New Excel File",
        "Append to Existing Sheet",
        "Create New Sheet in Existing File",
    ...
    if st.session_state.last_export_option != export_option:
        if export_option not in excel_export_modes:
            st.session_state.excel_file = None
        st.session_state.last_export_option = export_option
    ...
    if export_option == "Create New Excel File":
        ...
        if st.button("Export New Excel File", key="btn_export_new"):
            export_to_backend({
    ...
    # 2️⃣ APPEND TO EXISTING SHEET
        ...
        if not excel_files:
            ...
            if st.button("Append to Sheet", key="btn_append"):
                export_to_backend({
                    "processed_resumes": success_items,
                    "mode": "append_sheet",
    ...
    # DOWNLOAD BUTTON - show ONLY for Excel export modes and only if export succeeded
    if (export_option in excel_export_modes
            and st.session_state.get("excel_file")
            and st.session_state.excel_file.get("excel_file")):
